...
CREATE POLICY "Anyone can view posts" ON public.memorial_posts FOR SELECT USING (true);
CREATE POLICY "Auth users can create posts" ON public.memorial_posts FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can delete own posts" ON public.memorial_posts FOR DELETE USING (auth.uid() = user_id);
CREATE POLICY "Users can update own posts" ON public.memorial_posts FOR UPDATE USING (auth.uid() = user_id);
...
CREATE POLICY "Anyone can view comments" ON public.memorial_comments FOR SELECT USING (true);
CREATE POLICY "Auth users can create comments" ON public.memorial_comments FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can delete own comments" ON public.memorial_comments FOR DELETE USING (auth.uid() = user_id);

-- =========================
-- TABLE: memorial_likes
-- =========================
...
CREATE POLICY "Anyone can view likes" ON public.memorial_likes FOR SELECT USING (true);
CREATE POLICY "Auth users can like" ON public.memorial_likes FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can unlike" ON public.memorial_likes FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "Anyone can view memorial media" ON public.memorial_media FOR SELECT USING (true);
...
CREATE POLICY "Anyone can view memorial entries" ON public.memorial_entries FOR SELECT USING (true);
...
CREATE POLICY "Anyone can view memorial tributes" ON public.memorial_tributes FOR SELECT USING (true);
...
CREATE POLICY "Users can delete own tributes" ON public.memorial_tributes FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "Anyone can view candles" ON public.memorial_candles FOR SELECT USING (true);
...
CREATE POLICY "Anyone can view timelines" ON public.memorial_timelines FOR SELECT USING (true);
CREATE POLICY "Users can create own timelines" ON public.memorial_timelines FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can delete own timelines" ON public.memorial_timelines FOR DELETE USING (auth.uid() = user_id);
CREATE POLICY "Users can update own timelines" ON public.memorial_timelines FOR UPDATE USING (auth.uid() = user_id);

-- =========================
-- TABLE: memorial_access
-- =========================
...
CREATE POLICY "Users can view own access grants" ON public.memorial_access FOR SELECT USING (user_id = auth.uid());

-- =========================
-- TABLE: memorial_invitations
-- =========================
...
CREATE POLICY "Auth users can create invitations" ON public.memorial_invitations FOR INSERT WITH CHECK (auth.uid() = invited_by);
CREATE POLICY "Invitees can view their invitations" ON public.memorial_invitations FOR SELECT
USING (invitee_email = (SELECT profiles.email FROM profiles WHERE profiles.id = auth.uid()));
CREATE POLICY "Inviters can view own invitations" ON public.memorial_invitations FOR SELECT USING (invited_by = auth.uid());
...
CREATE POLICY "Public diary entries viewable" ON public.diary_entries FOR SELECT USING (is_private = false);
CREATE POLICY "Users can create own diary" ON public.diary_entries FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Users can create own trees" ON public.trees FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update own trees" ON public.trees FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete own trees" ON public.trees FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "Tree owners can manage access" ON public.tree_access FOR ALL USING (is_tree_owner(tree_id, auth.uid()));
CREATE POLICY "Users can view own tree access" ON public.tree_access FOR SELECT USING (user_id = auth.uid());

-- =========================
-- TABLE: tree_contributions
-- =========================
...
CREATE POLICY "Create contributions" ON public.tree_contributions FOR INSERT WITH CHECK (auth.uid() = contributor_id);
...
CREATE POLICY "Anyone can view templates" ON public.site_templates FOR SELECT USING (true);
CREATE POLICY "Creators can insert own templates" ON public.site_templates FOR INSERT WITH CHECK ((auth.uid() = creator_id) OR (creator_id IS NULL));
CREATE POLICY "Creators can update own templates" ON public.site_templates FOR UPDATE USING (auth.uid() = creator_id);
CREATE POLICY "Creators can delete own templates" ON public.site_templates FOR DELETE USING (auth.uid() = creator_id);
...
CREATE POLICY "Admins can view all creators" ON public.template_creators FOR SELECT USING (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Users can view own creator profile" ON public.template_creators FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can create own creator profile" ON public.template_creators FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Admins can update creators" ON public.template_creators FOR UPDATE USING (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Admins can delete applications" ON public.template_creators FOR DELETE USING (has_role(auth.uid(), 'admin'::app_role));
...
CREATE POLICY "Auth users can create purchases" ON public.template_purchases FOR INSERT WITH CHECK (auth.uid() = buyer_id);
CREATE POLICY "Users can update own purchases" ON public.template_purchases FOR UPDATE USING (auth.uid() = buyer_id);
...
CREATE POLICY "Users can view own payouts" ON public.creator_payouts FOR SELECT USING (auth.uid() = creator_id);
CREATE POLICY "Users can create own payouts" ON public.creator_payouts FOR INSERT WITH CHECK (auth.uid() = creator_id);
...
CREATE POLICY "Anyone can view guest tributes" ON public.tributes FOR SELECT USING (true);
CREATE POLICY "Anyone can create guest tributes" ON public.tributes FOR INSERT WITH CHECK (tribute_text IS NOT NULL AND char_length(tribute_text) > 0);
